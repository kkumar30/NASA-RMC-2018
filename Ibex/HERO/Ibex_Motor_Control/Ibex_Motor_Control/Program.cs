...
namespace Ibex_Motor_Control
{
    public class Program
    {
        ...
        private const int RESET_CONVEYOR_ENC = 3;
        ...
        private const int conveyor_ID = 4;
        ...
        {
            //Set up the motors
            CTRE.TalonSrx leftDriveMotor = new CTRE.TalonSrx(1);
            CTRE.TalonSrx rightDriveMotor = new CTRE.TalonSrx(2);
            CTRE.TalonSrx scoopMotor = new CTRE.TalonSrx(3);
            CTRE.TalonSrx depthMotor = new CTRE.TalonSrx(4);
            CTRE.TalonSrx conveyorMotor = new CTRE.TalonSrx(5);

            //Set encoders for each motor
            leftDriveMotor.SetFeedbackDevice(CTRE.TalonSrx.FeedbackDevice.QuadEncoder);
            rightDriveMotor.SetFeedbackDevice(CTRE.TalonSrx.FeedbackDevice.QuadEncoder);
            ...
            conveyorMotor.SetFeedbackDevice(CTRE.TalonSrx.FeedbackDevice.QuadEncoder);

            //Set direction of the encoders
            leftDriveMotor.SetSensorDirection(false);
            rightDriveMotor.SetSensorDirection(false);
            scoopMotor.SetSensorDirection(false);
            depthMotor.SetSensorDirection(false);
            conveyorMotor.SetSensorDirection(false);

            //Set Ticks per Rev of the encoders
            leftDriveMotor.ConfigEncoderCodesPerRev(80);
            rightDriveMotor.ConfigEncoderCodesPerRev(80);
            scoopMotor.ConfigEncoderCodesPerRev(80);
            depthMotor.ConfigEncoderCodesPerRev(80);
            conveyorMotor.ConfigEncoderCodesPerRev(80);

            //Sets PIDF values for each motor//
            leftDriveMotor.SetP(0, 0.35F);
            leftDriveMotor.SetI(0, 0.0F);
            leftDriveMotor.SetD(0, 0.0F);
            leftDriveMotor.SetF(0, 0.0F);
            leftDriveMotor.SelectProfileSlot(0);

            rightDriveMotor.SetP(0, 0.35F);
            rightDriveMotor.SetI(0, 0.0F);
            rightDriveMotor.SetD(0, 0.0F);
            rightDriveMotor.SetF(0, 0.0F);
            rightDriveMotor.SelectProfileSlot(0);
            ...
            conveyorMotor.SetP(0, 0.6F);
            conveyorMotor.SetI(0, 0.0F);
            conveyorMotor.SetD(0, 0.0F);
            conveyorMotor.SetF(0, 0.0F);
            conveyorMotor.SelectProfileSlot(0);

            //Sets Nominal Output Voltage for each motor
            leftDriveMotor.ConfigNominalOutputVoltage(+0.0F, -0.0F);
            rightDriveMotor.ConfigNominalOutputVoltage(+0.0F, -0.0F);
            scoopMotor.ConfigNominalOutputVoltage(+0.0F, -0.0F);
            depthMotor.ConfigNominalOutputVoltage(+0.0F, -0.0F);
            conveyorMotor.ConfigNominalOutputVoltage(+0.0F, -0.0F);

            // Set allowed error for closed loop feedback
            leftDriveMotor.SetAllowableClosedLoopErr(0, 0);
            rightDriveMotor.SetAllowableClosedLoopErr(0, 0);
            scoopMotor.SetAllowableClosedLoopErr(0, 0);
            depthMotor.SetAllowableClosedLoopErr(0, 0);
            conveyorMotor.SetAllowableClosedLoopErr(0, 0);

            //Set Initial position of the motors
            leftDriveMotor.SetPosition(0);
            rightDriveMotor.SetPosition(0);
             scoopMotor.SetPosition(0);
             depthMotor.SetPosition(0);
             conveyorMotor.SetPosition(0);

            //Sets Voltage Ramp rate of each motor
            leftDriveMotor.SetVoltageRampRate(0);
            rightDriveMotor.SetVoltageRampRate(0);
            scoopMotor.SetVoltageRampRate(0);
            depthMotor.SetVoltageRampRate(0);
            conveyorMotor.SetVoltageRampRate(0);
            ...
            talons.Add(leftDriveMotor);
            talons.Add(rightDriveMotor);
            talons.Add(scoopMotor);
            talons.Add(depthMotor);
            talons.Add(conveyorMotor);
            ...
            SetpointData leftDriveMotorSetpointData = new SetpointData(1, 0, 0.0F);
            SetpointData rightDriveMotorSetpointData = new SetpointData(2, 0, 0.0F);
            ...
            SetpointData conveyorMotorSetpointData = new SetpointData(5, 0, 0.0F);

            StatusData leftDriveMotorStatusData = new StatusData(1, leftDriveMotor);
            StatusData rightDriveMotorStatusData = new StatusData(2, rightDriveMotor);
            StatusData scoopMotorStatusData = new StatusData(3, scoopMotor);
            StatusData depthMotorStatusData = new StatusData(4, depthMotor);
            StatusData conveyorMotorStatusData = new StatusData(5, conveyorMotor);

            motorSetpointData.Add(leftDriveMotorSetpointData);
            motorSetpointData.Add(rightDriveMotorSetpointData);
            motorSetpointData.Add(scoopMotorSetpointData);
            motorSetpointData.Add(depthMotorSetpointData);
            motorSetpointData.Add(conveyorMotorSetpointData);

            motorStatusData.Add(leftDriveMotorStatusData);
            motorStatusData.Add(rightDriveMotorStatusData);
            motorStatusData.Add(scoopMotorStatusData);
            motorStatusData.Add(depthMotorStatusData);
            motorStatusData.Add(conveyorMotorStatusData);*/
            //////////////////////////////////////////
        ...
        private static void resetEncoderPositions(ArrayList talons)
        {
            ...
            */
            ...
            if (flags[RESET_CONVEYOR_ENC])
            {
                flags[RESET_CONVEYOR_msg)
        {
            ...
            if (msg.Trim().Equals("<ResetConveyorEncoder>"))
            {
                flags[RESET_CONVEYOR_ENC] = true;
            }
        }
